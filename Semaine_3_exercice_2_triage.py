...
# Semaine 3 - Exrcice 2 : Trage Patient Urgences CHU Brazzaville
...
age_patient = int(input("Age (Annee) :"))
...
if temperature > 39.5 or spo2 <90 or tension_syst > 180:
    ...
    coulleur_triage ='[ROUGE]'
    delai_pec = '< 0 minute'
    action_triage = 'Meedecin present immediament - code ROUGE active'

# Niveau 2 : URGEENT - conditions moins critiques mais toujours urgentes
elif temperature > 38.5 or spo2 < 94 or tension_syst > 140:
    niveau_triage = '2 - URGENT'
    coulleur_triage ='[ORANGE]'
...
# Niveau 4 : Moins URGENT
    ...
    coulleur_triage ='[VERT]'
    delai_pec = '<120 minutes'
    action_triage = 'File d\'attente standard'

# --- AFFICHAGE DU TRIAGE (S2 reutilise : f-strings) ---
print()
print("=" * 65)
print(f" RESULTAT DU TRIAGE - {nom_patient.upper()}")
print("=" * 65)
print(f" PARAMETRES VITAUX")
print(f" Temperature                  : {temperature} C  [{"ANORMAL - > 39.5" if temperature > 39.5 else "NORMAL"}]")
print(f" Saturation O2                : {spo2} %  [{"NORMAL" if spo2 < 90 else "NORMAL"}]")
print(f" Tension systolique           : {tension_syst} mmHg [{"ANORMAL" if tension_syst > 180 else "NORMAL"}]")
print(f" Douleur                      : {douleur} /10   [{"NORMAL" if douleur > 6 else "NORMAL"}]")
print("-" * 65)
print(f" NiVEAU DE TIRAGE             : {niveau_triage}")
print(f" COULEUR                      : {coulleur_triage}")
print(f" PRISE EN CHARGE              : {delai_pec}")
print(f" ACTION                       : {action_triage}") 
print("-" * 65)
print(f" Motif principal             : Temperatur {temperature} C > seuil 39.5 C")
print("=" * 65)